...
async function fetchCertificatesForWorkspace(user, projectIds) {
    ...
    const { entityTypes, pendingFilter, params, roleParamIndex } = buildCertificateScopeFilter(user);
    const queryParams = [projectIds, ...params];
    const rolePlaceholder = roleParamIndex > 0 ? `$${roleParamIndex + 1}` : null;
    const scopedPendingFilter = rolePlaceholder
        ? pendingFilter.replace('$ROLEIDX', rolePlaceholder)
        : pendingFilter;
...
function buildCertificateScopeFilter(user) {
...
async function countChiefEngineerApprovedPendingCerts(user, projectIds) {
    ...
    const result = await pool.query(
        `SELECT COUNT(*)::int AS count
         ...
         WHERE COALESCE(c.voided, false) = false
           AND c."projectId" = ANY($1::int[])
           AND ar.request_id IS NOT NULL
           ${scopedPendingFilter}
           AND EXISTS (
             SELECT 1 FROM approval_step_instances prev
             LEFT JOIN roles r ON r.roleid = prev.role_id
