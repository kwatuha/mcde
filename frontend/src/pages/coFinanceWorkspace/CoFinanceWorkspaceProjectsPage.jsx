...
import {
  ...
  projectTabLink,
  scopeChip,
} from './coFinanceWorkspaceShared';
import { useCoFinanceWorkspaceData } from './useCoFinanceWorkspaceData';

export default function CoFinanceWorkspaceProjectsPage() {
  const { hasPrivilege } = useAuth();
  const navigate = useNavigate();
  const { loading, error, load, search, setSearch, projects } = useCoFinanceWorkspaceData({
    include: 'projects',
  });
                  ...
                  <TableRow key={row.projectId} hover>
                    ...
                    <TableCell align="right">
                      <Stack direction="row" spacing={0.5} justifyContent="flex-end" flexWrap="wrap">
                        ...
                        <Button size="small" onClick={() => navigate(`${CO_FINANCE_WORKSPACE_ROUTES.progressPhotos}?projectId=${row.projectId}`)} startIcon={<PhotoCameraIcon />} sx={{ textTransform: 'none' }}>
                          Photos
                        </Button>
                        {canSetupScope ? (
                          <Button
