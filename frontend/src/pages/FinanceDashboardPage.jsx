...
const FinanceDashboardPage = () => {
  ...
  useEffect(() => {
    setFilters((prev) => {
      ...
      if (next.agency && !uniqueAgencies.includes(next.agency)) next.agency = '';
      const unchanged =
  ...
  const filteredProjects = useMemo(() => {
    return scopeBaseProjects.filter((p) => {
      ...
      if (filters.status && normalizeProjectStatus(p.status || p.Status || 'Unknown') !== filters.status) return false;
      return true;
    });
